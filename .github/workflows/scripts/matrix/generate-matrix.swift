...
struct Configuration {
    ...
    /// The Android SDK build, which is what the emulator tests run.
...
extension Configuration {
    ...
    /// first architecture configured rather than a different default from the tests.
...
/// A platform built and tested through xcodebuild, which is a step inside a macOS job rather
...
struct JobGroupSettings {
    ...
    ///
    /// An axis with one value contributes nothing, and the label leads: entry names are
    /// required status checks in adopting repositories, and a version and a label appended
...
struct MacOSMachines {
    ...
    /// The machines one swiftly toolchain runs on: an entry naming its own machine runs there
    /// alone, and on the architecture it names.
...
struct ContainerImage {
    ...
    /// What the image is tagged for after the toolchain, such as `noble`.
...
struct Toolchain {
    ...
    /// The image this toolchain runs in on a distribution, which upstream publishes a nightly
    /// under its own name.
