...
@Suite("The job-kind table")
struct JobKindTableTests {
  /// The kinds emitted by a block of their own: the platform sweeps, whose entries
  /// fan out over that platform's runners, and FreeBSD, whose entries carry a
  /// virtual machine rather than a toolchain.
  static let emittedByTheirOwnBlock: Set<String> = [
    "enable_linux",
    "enable_macos",
  ...
  private static func source() throws -> [String] {
    let contents = try String(
      contentsOf: URL(fileURLWithPath: Generator.scriptPath),
      encoding: .utf8
    )
    return contents.components(separatedBy: "\n")
  }

  /// The quoted arguments of one line, in order.
  private static func quotedArguments(of line: String) -> [String] {
  ...
  @Test("Every job kind the generator can report as enabled has a row")
  func everyKindHasARow() throws {
    // A kind with no row generates nothing. The generator only fails an empty
    // matrix, so a kind added to the enables and not to the table leaves a caller
    // who asked for it a green run with the job silently absent.
    let lines = try Self.source()

    var reported: Set<String> = []
    for line in lines {
      let trimmed = line.trimmingCharacters(in: .whitespaces)
      guard trimmed.hasPrefix("\"enable_"), trimmed.contains(":enable_") else { continue }
      let quoted = Self.quotedArguments(of: trimmed)
      guard let pair = quoted.first, let variable = pair.split(separator: ":").last else {
        continue
      }
      reported.insert(String(variable))
    }
    #expect(reported.count == 10, "the enables the generator reports on are \(reported)")

    var rows: [[String]] = []
    for line in lines where line.hasPrefix("emit_linux_job_kind ") {
      rows.append(Self.quotedArguments(of: line))
    }

    let tabled = Set(rows.compactMap { $0.first?.dropFirst() }.map(String.init))
    #expect(tabled == reported.subtracting(Self.emittedByTheirOwnBlock))
    for row in rows {
      #expect(row.count == Self.axesPerRow, "\(row.first ?? "a row") states \(row.count) axes")
      #expect(row.first?.hasPrefix("$") == true, "a row's first axis is the enable")
    }
  }

  @Test("Every kind in the table generates the entries it is enabled for")
  func everyRowGeneratesEntries() throws {
    // A row naming a version list no caller can reach, or an enable nothing sets,
    // would leave the kind unreachable while the table says it is covered.
    let enables = [
